...
jpostdb.slice_comparison = jpostdb.slice_comparison || {
	...
	for(let i = 0; i < param.apiArg.length; i++){
	    if(param.apiArg[i].match(/^togodb=http.+$/)){
		let endpoint = param.apiArg[i].match(/^togodb=(http.+)$/)[1];
		param.apiArg[i] = "togodb=" + decodeURIComponent(endpoint);
		userdata = "userdata_";
		param.userdataComparison = 1;
		break;
	    }
	}
	var url = jpostdb.api + userdata + "slice_comp_stats?" + param.apiArg.join("&");
	...
	var share = tr.append("td");
	share.style("color", "#007fb1").html(json.prot_share);
	if(json.prot_share == 0) share.style("color", "red");
	tr.append("td").html(json.prot2);
	tr.append("td").html(json.prot2_uniq);
	if(!param.userdataComparison){
	    tr = table.append("tr");
	...
	select.append("option").attr("value", "fc").html("Fold change of the mean")
	select.on("change", function(){
		...
		if(renderDiv.select("#enrich_list_div")) renderDiv.select("#enrich_list_div").remove();
		var arg = [];
		for(var i = 0; i< param.apiArg.length; i++){
		    if(param.apiArg[i].match(/^dataset/)) arg.push(param.apiArg[i]);
                    else if (param.apiArg[i] == "slice_stanza") arg.push(param.apiArg[i]);
		}
		var method = "sc";
	...
	param.tax = json[0].tax;
	
	var tableData = {
	    head: ["", "Protein name", "Accession", "ID", "Fold change", "log(fc)", "p-value"],
	    arg: ["_bgcolor", "name", "uniprot", "id", "foldchange", "logfc", "p_value"],
	    align: [0,0,0,0,1,1,1],
	    width: [20,"","","","","",""],
	    data: proteinList
	};
	...
	var selectionSwitch = function(){
	    ...
	    var opt = enrich.select("#chipatlas_comp_opt");
	...
	var plot = function(f){
		...
		var genes_b = [];
		for(var i = 0; i < proteinList.length;i++){
		    uniprots.push(proteinList[i].uniprot + ":" + proteinList[i].logfc);
		    if(proteinList[i].gene){
			genes.push(proteinList[i].gene);
			if(proteinList[i].logfc > 0) genes_a.push(proteinList[i].gene);
			else genes_b.push(proteinList[i].gene);
		    }
		}
		param.uniprots = uniprots.join("_");
		param.genes = genes;
		param.genes_a = genes_a;
		param.genes_b = genes_b;
	...
	var render = function(){
		...
		.on("change", function(){
		    if(!this.value.match(/^--/)){
			if(this.value.match(/chipatlas/)){
			    var chip_form = enrich_head.select("#chip_form");
			    chip_form.html = "";
			    chip_form.append("input")
				.attr("name", "taxonomy")
				.attr("value", param.tax)
				.attr("type", "hidden");
			    if(this.value.match(/chipatlas_comp/)){
				chip_form.select("#chipatlas_genes").remove();
				var genes_a = chip_form.append("input")
				    .attr("name",  "genesetA")
				    .attr("id",  "chipatlas_genes_a")
				    .attr("value", param.genes_a.join("\n"))  // bug of d3.js for firefox (not insert "\n")
				    .attr("type", "hidden");
				genes_a.node().value =  param.genes_a.join("\n"); // avoid bug of d3.js for firefox
				var genes_b = chip_form.append("input")
				    .attr("name",  "genesetB")
				    .attr("id",  "chipatlas_genes_b")
				    .attr("value", param.genes_b.join("\n"))  // bug of d3.js for firefox (not insert "\n")
				    .attr("type", "hidden");
				genes_b.node().value =  param.genes_b.join("\n"); // avoid bug of d3.js for firefox;
			//	console.log("genesetA\n" + param.genes_a.join("\n"))
			//	console.log("genesetB\n" + param.genes_b.join("\n"))
			    }else{
				chip_form.select("#chipatlas_genes_a").remove();
				chip_form.select("#chipatlas_genes_b").remove();
				var genes_input = chip_form.append("input")
				    .attr("name",  "genes")
				    .attr("id",  "chipatlas_genes")
				    .attr("value", param.genes.join("\n"))  // bug of d3.js for firefox (not insert "\n")
				    .attr("type", "hidden");
				genes_input.node().value =  param.genes.join("\n"); // avoid bug of d3.js for firefox
			//	console.log("geneset\n" + param.genes.join("\n"))
			    }
			    var chip_form = enrich_head.select("#chip_form");
			    chip_form.attr("target", "chip_form");
			    chip_form.node().submit();
			}else{
			    enrich_svg.transition().duration(param.anime).attr("height", 0);
			    var g = enrich_svg.selectAll(".forcegraph");
			    g.remove();
			    var list = enrich_table.select("#enrich_list_div");
			    list.remove();
			    jpostdb.fetchReq("post", url, "data=" + param.uniprots + "&e=1&target=" + this.value, renderDiv, param.width, group.enrichMap);
			}
		    }
		});
	    ...
	    if(param.tax == "9606"      // human
	       || param.tax == "10090"  // mouse
	       || param.tax == "10116"  // rat
	       || param.tax == "7227"   // Drosophila
	       || param.tax == "6239"   // C. elegans
	       || param.tax == "4932"){ // yeast
		select.append("option")
		    .attr("value", "chipatlas")
		    .text("ChIP-seq in ChIP-Atlas");
		select.append("option")
		    .attr("id", "chipatlas_comp_opt")
		    .attr("value", "chipatlas_comp")
		    .text("ChIP-seq up/down comparison");
		var chip_form = enrich_head.append("form")
		    .attr("id", "chip_form")
		    .attr("action", "https://chip-atlas.org/enrichment_analysis")
		    .attr("method", "post")
		    .attr("display", "none");
	    }
	...
	var arg = ["_bgcolor", "term_label", "count", "pvalue", "qvalue"];
	var head = ["", "term", "# proteins", "p-value", "q-value"];
	var align = [ 0, 0, 1, 1, 1];
